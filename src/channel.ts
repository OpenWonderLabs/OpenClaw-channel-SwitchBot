...
class SwitchBotChannel {
  ...
  private async pushDeviceEventToChat(event: SwitchBotDeviceEvent): Promise<void> {
    try {
      ...
      const timestamp = new Date(ctx.timeOfSample || Date.now()).toLocaleString('en-US', { timeZone: 'Asia/Shanghai' });
      ...
      const messageBody = [
        ...
        ``,
        `Please analyze this smart home device status update and tell me what happened in a concise and natural way. If there is anything abnormal (such as doors/windows left open for a long time or motion detected at unusual hours), please highlight it.`,
      ].join('\n');
  ...
  private extractImageUrls(ctx: Record<string, unknown>): Array<{ url: string; label: string }> {
    ...
    const imageFields: Array<{ key: string; label: string }> = [
      { key: 'imageUrl', label: 'Device Snapshot' },
      { key: 'detectionUrl', label: 'Detection Event Image' },
      { key: 'thumbnailUrl', label: 'Thumbnail' },
      { key: 'imgUrl', label: 'Image' },
      { key: 'photoUrl', label: 'Photo' },
      { key: 'snapshotUrl', label: 'Snapshot' },
      { key: 'picUrl', label: 'Image' },
      { key: 'image', label: 'Image' },
      { key: 'snapshot', label: 'Snapshot' },
      { key: 'url', label: 'Linked Image' },
    ];
  ...
  private async downloadEventImages(
    ...
    for (const img of images) {
      try {
        ...
        const filename = `${deviceMac}_${timestamp}_${img.label.replace(/[^a-zA-Z0-9]/g, '_')}${ext}`;
